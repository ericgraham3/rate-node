...
module RateNode
  module Seeds
    class Rates
        ...
        schedule_data = SCHEDULE_OF_RATES.map do |row|
          {
            ...
            base: row[:rate] * 100,
            per_thousand: nil,  # CA uses flat rates, not per-thousand
            elc: row[:elc] * 100
          }
        end
      ...
      def self.seed_ca_trg_cpl_rates
        # CA CPL rates are empty/not used
        Models::CPLRate.seed(CA_TRG_CPL_RATES, state_code: "CA", underwriter_code: "TRG", effective_date: CA_TRG_EFFECTIVE_DATE, expires_date: nil) unless CA_TRG_CPL_RATES.empty?
      end

      def self.seed_nc_trg
        seed_nc_trg_rate_tiers
        seed_nc_trg_refinance_rates
        seed_nc_trg_policy_types
        seed_nc_trg_endorsements
        seed_nc_trg_cpl_rates
      end

      def self.seed_nc_trg_rate_tiers
        schedule_data = NC_TRG_RATE_TIERS.map do |row|
          {
            min: row[:min] * 100,
            max: row[:max] ? row[:max] * 100 : nil,
            base: row[:rate] * 100,
            per_thousand: row[:per_thousand],  # Use per_thousand from data
            elc: row[:elc] * 100
          }
        end

        Models::RateTier.seed(schedule_data, state_code: "NC", underwriter_code: "TRG", effective_date: NC_TRG_EFFECTIVE_DATE, expires_date: nil)
      end

      def self.seed_nc_trg_refinance_rates
        data = NC_TRG_REFINANCE_RATES.map do |row|
          {
            min: row[:min] * 100,
            max: row[:max] ? row[:max] * 100 : nil,
            rate: row[:rate] * 100
        ...
        Models::RefinanceRate.seed(data, state_code: "NC", underwriter_code: "TRG", effective_date: NC_TRG_EFFECTIVE_DATE, expires_date: nil)
      end

      def self.seed_nc_trg_policy_types
        Models::PolicyType.seed(state_code: "NC", underwriter_code: "TRG", effective_date: NC_TRG_EFFECTIVE_DATE, expires_date: nil)
      end

      def self.seed_nc_trg_endorsements
        Models::Endorsement.seed(NC_TRG_ENDORSEMENTS, state_code: "NC", underwriter_code: "TRG", effective_date: NC_TRG_EFFECTIVE_DATE, expires_date: nil)
      end
      ...
      # North Carolina - Chicago Title Rates (Effective October 1, 2025)
      ...
      # CPL (Closing Protection Letter) Rates - Chicago Title NC (Effective October 1, 2025)
      ...
      # CA CPL: No tiered rates (handled as flat $25 fee in calculator if needed)
      CA_TRG_CPL_RATES = [].freeze
    end
  end
end
