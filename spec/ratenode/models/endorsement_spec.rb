...
RSpec.describe RateNode::Models::Endorsement do
  describe "#calculate_premium" do
    ...
    context "no charge endorsement" do
      let(:endorsement) { described_class.find_by_code("ALTA 4.1", state: "CA", underwriter: "TRG") }

      it "returns zero" do
        expect(endorsement.calculate_premium(50_000_000, state: "CA", underwriter: "TRG")).to eq(0)
      end
    end

    context "percentage endorsement" do
      let(:endorsement) { described_class.find_by_code("CLTA 100", state: "CA", underwriter: "TRG") }

      it "calculates percentage of Schedule of Rates, not liability" do
        premium = endorsement.calculate_premium(50_000_000, state: "CA", underwriter: "TRG")
        expect(premium).to eq(47_130)
      end
    end

    context "percentage endorsement with minimum" do
      let(:endorsement) { described_class.find_by_code("CLTA 123.1", state: "CA", underwriter: "TRG") }

      it "enforces minimum charge" do
        premium = endorsement.calculate_premium(2_000_000, state: "CA", underwriter: "TRG")
