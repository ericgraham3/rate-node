...
module TitleRound
  module Models
    class Endorsement
      PRICING_TYPES = %w[flat percentage tiered no_charge].freeze

      attr_reader :id, :code, :name, :pricing_type, :base_amount_cents,
      ...
      def self.find_by_code(code, state:, underwriter:, as_of_date: Date.today)
        ...
        row = Database.instance.get_first_row(
          ...
          [code.upcase.strip, state, underwriter, as_of_date_str, as_of_date_str]
        )
        row ? new(row) : nil
      end
      ...
      def self.seed(data, state_code:, underwriter_code:, effective_date:, expires_date: nil)
        ...
        data.each do |row|
          db.execute(
            "INSERT OR IGNORE INTO endorsements (code, name, pricing_type, base_amount_cents, percentage, min_cents, max_cents, concurrent_discount_pct, owner_only, lender_only, notes, state_code, underwriter_code, effective_date, expires_date) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)",
            [row[:code], row[:name], row[:pricing_type], row[:base_amount], row[:percentage], row[:min], row[:max], row[:concurrent_discount], row[:owner_only] ? 1 : 0, row[:lender_only] ? 1 : 0, row[:notes], state_code, underwriter_code, effective_date_str, expires_date_str]
          )
        end
      end
      ...
      def calculate_percentage_premium(liability_cents, state:, underwriter:, as_of_date: Date.today)
        ...
        base_rate = RateTier.calculate_rate(liability_cents, state: state, underwriter: underwriter, as_of_date: as_of_date)
