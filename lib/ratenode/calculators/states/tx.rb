...
module RateNode
  module States
    # Texas-specific rate calculator.
    #
    # TX uses formula-based rates for policies over $100,000.
    # TX does NOT round liabilities - exact amounts are used.
    #
    class TX < BaseStateCalculator
      # Calculate owner's title insurance premium for Texas.
      #
      ...
      # Calculate lender's title insurance premium for Texas.
      #
      # @param params [Hash] Calculation inputs
      ...
      def calculate_lenders_premium(params)
        ...
        rules = rules_for("TX", underwriter: underwriter)
      ...
      # Get reissue discount amount (TX does not support reissue discounts).
      #
      # @param params [Hash] Calculation parameters
      # @return [Integer] Always 0 for TX
      #
      ...
      def state_rules
        @state_rules ||= rules_for("TX", underwriter: @underwriter)
      end

      def calculate_standard
        # TX does NOT round liabilities
        ...
        multiplier = Models::PolicyType.multiplier_for(@policy_type, state: "TX", underwriter: @underwriter, as_of_date: @as_of_date)
